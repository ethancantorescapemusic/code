<CsoundSynthesizer>
    ...
    <CsInstruments>
        ...
        ; massign 1, "triggerInst"    ; this might need to be added back in. I have no idea why it works without it to be honest
        ...
        */
        instr arpegiator
            ...
            /**
                ===================== LOOP SECTION ============================
                if i was being more careful I would have named this instrument 
                "looper" or something like that because its the central loop
                that runs both the arp as well as the drum machine.
                I think i would have also broken the drum machine and arp into 
                their own opcodes that are called from here rather than have
                the entire thing shoved into this.
            
                From what I understand about timout is that it starts at clock
                then goes to arp then waits until the iNoteLength to go back to 
                clock etc. To me its less of a loop and more like a repeat. At
                least that is how I conceptualize it in terms of music vs code
            */
            clock:          
            ...
            arp:
                ...
                /** ================ DRUM MACHINE ======================      
                    ...
                    Idealy it would be in its own file in an opcode that is called each time the loop
                    loops but I didn't feel like doing that so this is fine. It just makes this file
                    less readable
                */
                ...
                /**
                    ...
                    it is a bad solution because it will not work if you switch between patterns
                    of the same length but I could not figure out a good way to compare arrays
                    without stepping through both of them. I think that might have to be the 
                    solution if anyone wants to continue this project.
                */
        ...
        /** =============================== TRIGGER INSTRUMENT =============================================
            ...
            This is where i had all of the problems for the most part of this project. I think the main issue
            was that I needed to change everything to k rate and took all of the array processing things into the
            loop. It turns out you cannot change the size of arrays at k rate at all so the whole idea needed to
            get swapped up to the three large arrays instead.
         */
        instr triggerInst
            kStatus, kChan, kData1, kData2 midiin               ; recieve midi all of the midi information.
                                                                ; I almost used CPSmidi and Ampmidi but the 
                                                                ; problem with those is i cannot get info from
                                                                ; modwheels and other stuff
            ...
            if kSTrigger == 1 || kCTrigger == 1 || kD1Trigger == 1 || kD2Trigger == 1 then      ; check to see if any of the triggers have popped
                ...
                if(kData1 == giMOD2) then       ; DRUM PATTERN SELECT KNOB
                    gkDrumSelect = round((kData2 / 128) * (giNumDrumPatterns - 1))              ; this is a fun little rounding algorithm i thought up
                                                                                                ...
                                                                                                ; number of selections (in this case drum patterns). I don't
                                                                                                ; think its exactly dividing the knob up equally but it works 
                                                                                                ; enough for me
